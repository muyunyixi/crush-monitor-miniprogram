...
Page({
  ...
  async chooseScreenshot() {
    ...
    for(let i=0;i<files.length;i++) {
      ...
      try{
        ...
        const task={ jobId:uploaded.jobId,jobToken:uploaded.jobToken||'',conversationId,imageIndex:i,batchId:store.uid(),status:'waiting',path:files[i].tempFilePath,createdAt:Date.now() };
      ...
      }catch(error){this.setData({status:`第 ${i+1} 张失败：${error.message||error.errMsg||'网络错误'}。已完成 ${completed} 张；其余继续处理。`});}
    }
  ...
  async finishTask(task) {
    ...
    try{const info=await new Promise((resolve,reject)=>wx.getImageInfo({src:task.path,success:resolve,fail:reject}));const parsed=parseScreenshot(result.items,info.width,info.height);text=parsed.text;title=parsed.title;uncertain=parsed.uncertain;}catch{ text=(result.lines||[]).map(line=>`待确认：${line}`).join('\n');uncertain=(result.lines||[]).length; }
  ...
  async resumeTasks() { if(!this.state||this.resuming)return;this.resuming=true;
    for(const c of this.state.conversations)for(const task of c.tasks||[]) {
      ...
      try{await this.finishTask(task);}catch(error){this.setData({status:`识字任务查询暂时失败：${error.message||'网络错误'}。下次进入会继续查询。`});}
    }this.resuming=false;
  },
