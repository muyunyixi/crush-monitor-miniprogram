...
function request(path, options = {}) {
  return new Promise((resolve, reject) => wx.request({ url: BASE + path, timeout: 25000, ...options,
    success: r => r.statusCode >= 200 && r.statusCode < 300 ? resolve(r.data) : reject(new Error(r.data?.error || `服务返回 ${r.statusCode}`)), fail: reject }));
}
...
async function poll(task) {
  for (let i = 0; i < 26; i++) {
    ...
    if (data.done) { if (data.status !== 200) throw new Error(data.result?.error || '识字失败。'); return data.result; }
  }
